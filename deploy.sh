...
scp -i "id_rsa" -r StrictHostKeyChecking=no -r "$/var/lib/jenkins/workspace/Final_Project/docker-compose.yml" ec2-user@${MACHINE}:~

scp -i "id_rsa" -r StrictHostKeyChecking=no ec2-user@${MACHINE} << 'EOF'
